...
export class API {
    ...
    constructor(clientIds: string[] = [], public oauthToken?: string, proxy?: string) {
    ...
    public getURL = (URI: string, params?: Record<string, any>) => {
        ...
        if (this.currentClientId) options.query.client_id = this.currentClientId;
    ...
    private readonly getRequest = async (pool: Pool, origin: string, endpoint: string, params?: Record<string, any>) => {
        if (!this.currentClientId) await this.getClientId();
    ...
    public getClientId = async (reset?: boolean) => {
        if (!this.oauthToken && (!this.clientIds.length || reset)) {
            const newClientId = await this.getClientIdWeb().catch(webError =>
